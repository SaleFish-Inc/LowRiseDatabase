...
--WITH SCHEMABINDING
as
SELECT DISTINCT [firstname] AS FirstName, [lastname] AS LastName, [birthday] AS BirthDay, [address] AS Address, [city] AS CityName, [postalcode] as Zip, 
...
WHERE [p].[firstname] IS NOT NULL AND [p].[lastname] IS NOT NULL AND
	  ...
	  ISNUMERIC([firstname]) = 0 and firstname not like '%[0123456789@\_]%' 
go
